...
export default async function handler(request, response) {
  ...
  if (request.method === 'GET' && (request.query?.clean_abandoned === 'true' || request.query?.sweep === 'true')) {
    try {
      ...
      console.log(`[BLOB_SWEEP_COMPLETED] Deleted ${deleted.length} abandoned temporary Blob objects.`);
  ...
  try {
    ...
    console.log(`[TEMPORARY_BLOB_DELETED] Successfully deleted temporary Blob object.`);
